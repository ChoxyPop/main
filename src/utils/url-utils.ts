...
export function getTagUrl(tag: string): string {
	if (!tag) return url("/archivo/");
	return url(`/archivo/?tag=${encodeURIComponent(tag.trim())}`);
}
...
export function getCategoryUrl(category: string | null): string {
	if (
		...
		return url("/archivo/?uncategorized=true");
	return url(`/archivo/?category=${encodeURIComponent(category.trim())}`);
}
